  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => { setShowAddForm(true); setNewAssetType('extintor'); }} className="bg-[#af101a] text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-red-700 transition-colors border-none cursor-pointer">
          ➕ Novo Extintor
        </button>
      </div>
        ...
        {extintores.map((asset) => (
          <div key={asset.id} className="bg-white rounded-2xl border border-[#CFD8DC] shadow-sm relative overflow-hidden flex flex-col justify-between hover:shadow-lg transition-shadow group">
            ...
            <div className="border-t border-slate-100 bg-slate-50/50 p-3 flex justify-between gap-2 overflow-x-auto shrink-0">
              ...
              <button onClick={() => { setSelectedAssetForHistory({ ...asset, type: 'extintor' }); }} className="border border-slate-200 hover:bg-slate-100 text-slate-750 font-bold px-2 py-1 rounded-lg text-[10px] uppercase flex items-center gap-1 shrink-0 bg-white cursor-pointer" title="Ver Histórico NBR">📜 Histórico</button>
              ...
              }} className="bg-red-50 text-red-600 p-2 rounded-lg hover:bg-red-100 border-none cursor-pointer" title="Excluir">❌</button>
            </div>
          </div>
        ))}
      </div>
