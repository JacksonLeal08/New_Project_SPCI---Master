        ...
        {sinalizacoes.map((asset) => (
          <div key={asset.id} className="bg-white rounded-2xl border border-[#CFD8DC] shadow-sm relative overflow-hidden flex flex-col justify-between hover:shadow-lg transition-shadow group">
            ...
            <div className={`flex flex-col justify-between h-full w-full transition-all duration-300 ${
              ...
              <div className="border-t border-slate-100 bg-slate-50/50 p-3 flex justify-between gap-2 overflow-x-auto shrink-0 font-['Hanken_Grotesk'] font-bold uppercase rounded-b-2xl">
                ...
                {canDelete && (
                  <button onClick={() => {
                    requestAssetDeletion(asset, 'sinalizacao', async () => {
                      setDeletingAssetId(asset.id);
                      await new Promise((resolve) => setTimeout(resolve, 1200));
                      await deleteAsset('sinalizacoes', asset.id);
